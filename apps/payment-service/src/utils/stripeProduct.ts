...
export const createStripeProduct = async (item: StripeProductType) => {
  try {
    const res = await strip.products.create({
      id: item.id,
      name: item.name,
      default_price_data: {
        currency: 'usd',
...
export const getStripeProductPrice = async (productId: number) => {
  try {
    const res = await strip.prices.list({ product: productId.toString() });
...
export const deleteStripeProduct = async (productId: number) => {
  try {
    const res = await strip.products.del(productId.toString());
    return res;
  } catch (error) {
    console.log(error);
    return error;
